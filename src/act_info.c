...
if (ch && ch->pcdata && ch->pcdata->clan && wch->pcdata->clan 
    ...
    if ( first_mortal )
    {
      if ( !ch )
      {
        sprintf(mudnamebuffer, "&z(  &rPlayers&z  )&R-_-&r^^-_-^^-&R_-&r^^-_-^^-_-^^-&R_-^^-&r_-^^-_-&R^^&z(=====+ &r%s &z+====&z=)&w\n\n", sysdata.mudname );
        fprintf( whoout,"%s", htmlcolor(mudnamebuffer) );
      }
    ...
    if ( first_imm )
    {
      if ( !ch )
	  {
          sprintf(mudnamebuffer, "&z( &rImmortals&z )&R-_&r-^^-_-&R^&r-_-^^-_-^^&R-_-^^&r-_-^^-_-^^-&R_-^^&z(=====+ &r%s &z+====&z=)&w\n\n", sysdata.mudname);
		  fprintf( whoout, "%s", htmlcolor(mudnamebuffer) );
	  }
